...
for T5_NAME in "${T5_LIST[@]}"; do
  ...
  for item in "${RQ_SETTINGS[@]}"; do
    ...
    RUN_OUT="${OUT_ROOT}/${TAG}/${RQ_TAG}"
    RUN_LOG="${LOG_ROOT}/${TAG}_${RQ_TAG}.log"
    PORT=$((BASE_PORT + idx))
...
cfg.rq_ckpt = f"dig/checkpoints/RQ/{RQ_TAG}/ckpt_last.pt"
...
PY
    ...
    # 3) Safety checks
    # --------------------------------------------------
    RQ_CKPT_ABS="${DATA_ROOT}/dig/checkpoints/RQ/${RQ_TAG}/ckpt_last.pt"
