...
class MBEIRListInstructioneDataset(MBEIRDatasetBase):
    ...
    def __getitem__(self, index):
        ...
        query_prompt_ids = torch.LongTensor(
            self.tokenizer(query_prompt, padding="max_length", max_length=42, truncation=True,
...
class MBEIRDictInstructioneDataset(MBEIRDatasetBase):
    ...
    def __getitem__(self, index):
        ...
        if self.tokenizer is not None:
            query_prompt = self._get_random_query_prompt(query_dataset_id, query_modality, pos_cand_modality)
            query_prompt_ids = torch.LongTensor(
                self.tokenizer(query_prompt, padding="max_length", truncation=True, add_special_tokens=False).input_ids)
            query_txt = format_string(query_txt)
...
class MBEIRDictInstructioneDataset(MBEIRDatasetBase):
    ...
    def _prepare_query_positive_pairs(self):
        ...
        for query_index, mbeir_entry in enumerate(self.query_data):
            ...
            if len(pos_cand_list) < self.num_positives:
                extended_pos_cand_list = pos_cand_list + random.choices(pos_cand_list,
                                                                        k=self.num_positives - len(pos_cand_list))
            else:
    ...
    def __getitem__(self, index):
        ...
        if self.tokenizer is not None:
            query_txt_id = torch.LongTensor(
                self.tokenizer(query_txt, padding="max_length", truncation=True, add_special_tokens=False).input_ids)
        ...
        for pos_cand_did, order in pos_cand_dids_with_order:
            ...
            pool['txt_emb'] = torch.cat(
                [pool['txt_emb'], self.pool_dict['text'][self.did_to_index[h_did]].unsqueeze(0)])
            pool['img_mask'] = torch.cat(
                [pool['img_mask'], self.pool_dict['img_mask'][self.did_to_index[h_did]].unsqueeze(0)])
            pool['txt_mask'] = torch.cat(
                [pool['txt_mask'], self.pool_dict['text_mask'][self.did_to_index[h_did]].unsqueeze(0)])

            if self.tokenizer is not None:
                pool_txt_id = torch.LongTensor(self.tokenizer(pos_cand_txt, padding="max_length", truncation=True,
                                                              add_special_tokens=False).input_ids)
        ...
        if self.return_instruct and self.tokenizer is not None:
            query_prompt = self._get_random_query_prompt(query_dataset_id, query_modality,
                                                         pos_cand.get("modality", None))
            instruct = torch.LongTensor(
                self.tokenizer(query_prompt, padding="max_length", truncation=True, add_special_tokens=False).input_ids)
            ...
            for instance_key in instance_keys:
                ...
                for item in items:
                    ...
                    if inst is not None and self.seq2seq_tokenizer is not None:
                        inst_id = self.seq2seq_tokenizer(inst, padding="max_length", max_length=75, truncation=True,
                                                         add_special_tokens=False).input_ids
                        txt_id = self.seq2seq_tokenizer(txt, padding="max_length", max_length=75, truncation=True,
                                                        add_special_tokens=False).input_ids
                    elif self.seq2seq_tokenizer is not None:
                        inst_id = self.seq2seq_tokenizer(" ", padding="max_length", max_length=75, truncation=True,
                                                         add_special_tokens=False).input_ids
                        txt_id = self.seq2seq_tokenizer(" ", padding="max_length", max_length=75, truncation=True,
                                                        add_special_tokens=False).input_ids
                    else:
                        inst_id = None
                        txt_id = None
