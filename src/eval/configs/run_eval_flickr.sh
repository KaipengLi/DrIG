...
CFG_PATH="${1:-${SCRIPT_DIR}/eval_flickr.yaml}"

BASE_RQ_YAML="${SRC}/models/residual_quantization/configs_scripts/train_rq_flickr.yaml"
...
for T5_NAME in "${T5_LIST[@]}"; do
  ...
  for item in "${RQ_SETTINGS[@]}"; do
    ...
    T5_CKPT_REL="checkpoints/DiG4UMR/flickr30k/${TAG}/${RQ_TAG}/finetune_t5_epoch_010.pt"
    RQ_DATASET_NAME="flickr"
    RQ_CKPT_REL="checkpoints/RQ/${RQ_DATASET_NAME}/${RQ_TAG}/ckpt_last.pt"
...
PY
    ...
    # --------------------------------------------------
    RUN_SAVE_DIR="${DIG_ROOT}/eval_outputs/${TAG}/${RQ_TAG}"
