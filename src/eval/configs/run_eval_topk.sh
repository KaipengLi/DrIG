...
PY
    ...
    for TOPK in "${GUIDE_TOPK_LISTS[@]}"; do
      TMP_EVAL_YAML="$(mktemp /tmp/eval_${TAG}_${RQ_TAG}_topk_${TOPK}_XXXX.yaml)"

      # --------------------------------------------------
      # B) Build per-run eval yaml
      #   save_dir:
      #   .../${TAG}/${RQ_TAG}/topk/topk_${TOPK}
      # --------------------------------------------------
      RUN_SAVE_DIR="${DIG_ROOT}/eval_outputs/${TAG}/${RQ_TAG}/topk/topk_${TOPK}"
...
cfg.guide_topk_list = GUIDE_TOPK
cfg.out_tsv = f"recall_results_topk_{GUIDE_TOPK}.tsv"
...
print("  guide_topk_list:", cfg.guide_topk_list)
...
PY
      ...
      for task in "${TASKS[@]}"; do
        for nb in "${NUM_BEAMS[@]}"; do
          ...
          echo "============================================================"
          echo ">>> model=${TAG} rq=${RQ_TAG} topk=${TOPK} task=${task} num_beams=${nb}"
